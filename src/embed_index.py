...
def _load_chunks_from_disk(processed_dir: pathlib.Path) -> List[Dict]:
    chunks_path = processed_dir / "chunks.jsonl"
    with open(chunks_path, "r", encoding="utf-8") as f:
        return [json.loads(line) for line in f]

def _embed_texts(texts: List[str], model: SentenceTransformer, batch: int = 128) -> np.ndarray:
    vecs = model.encode(texts, batch_size=batch, show_progress_bar=False, normalize_embeddings=True)
